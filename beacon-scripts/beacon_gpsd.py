...
## Updated 2019 Robert Middelmann VK5TRM <vk5trm@gmail.com>
...
class Beaconer(threading.Thread):
   ...
   def save_beacon(self,aprs_string): # Used for aprx, saves to file
       ...
       fout.truncate()
       fout.write(aprs_string+COMMENT)
       fout.close()
            ...
            while not self.stopped:
                ...
                if self.gps_status['class']=='TPV': # Do we have a GPS fix?
                   if self.gps_status['mode']==3: # Do we have 3D fix? (Not available unless in TPV)
                      ...
                      if math.trunc(time.time() - self.beacon_timer)>=self.beacon_period:
                         self.runbeacon()
   ...
   try:
      ...
      while 1:
         ...
         if shout.get_last_beacon()==0: # No beacon yet sent
            ...
            if APRX:
               print APRX_PATH, ' updated ', math.trunc(time.time() - shout.get_last_beacon()), '/', shout.get_beacon_period(), ' seconds.'
